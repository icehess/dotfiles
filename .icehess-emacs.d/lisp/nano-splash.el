  ...
  (let* ((splash-buffer  (get-buffer-create "*splash*"))
    ...
    (if (eq 0 (length (cl-loop for buf in (buffer-list)
        ...
        (with-current-buffer splash-buffer
          ...
          (insert-char ?\n padding-center)
          (insert (propertize "İ c e H Ξ S S / N Λ N O" 'face 'nano-face-strong))
          (center-line)
          (insert "\n")
          (insert (propertize "Emacs is journey my ass" 'face 'nano-face-faded))
          (center-line)
          ...
          ;; (run-with-idle-timer 2.0 nil 'nano-splash-fade-out-slow)
